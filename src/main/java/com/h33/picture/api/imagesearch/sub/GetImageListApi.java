package com.h33.picture.api.imagesearch.sub;
...
public class GetImageListApi {
    ...
    public static List<ImageSearchResult> getImageList(String url) {
        try {
            ...
            if (statusCode == 200) {
                ...
                throw new BusinessException(ErrorCode.OPERRATION_ERROR, "接口调用失败");
            }
        } catch (Exception e) {
            log.error("获取图片列表失败", e);
            throw new BusinessException(ErrorCode.OPERRATION_ERROR, "获取图片列表失败");
        }
    }
    ...
    private static List<ImageSearchResult> processResponse(String responseBody) {
        ...
        if (!jsonObject.containsKey("data")) {
            throw new BusinessException(ErrorCode.OPERRATION_ERROR, "未获取到图片列表");
        }
